...
def test_rtl_loop_parsing(mocker, mock_subprocess):
    ...
    calls_blacklist = [
        c for c in mock_proc_logic.dispatch_reading.call_args_list if "999" in str(c)
    ]
    ...
    calls_last_seen = [
        c for c in mock_proc_logic.dispatch_reading.call_args_list if c.args[1] == "last_seen"
    ]
...
def test_flatten_nested_json():
    ...
    input_data = {"id": 123, "flags": ["foo", "bar"], "channel": {"A": 1, "B": 2}}

    expected = {"id": 123, "flags_0": "foo", "flags_1": "bar", "channel_A": 1, "channel_B": 2}
