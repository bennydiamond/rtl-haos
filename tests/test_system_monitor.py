    ...
    while True:
        ...
        try:
            ...
            mqtt_handler.send_sensor(
                DEVICE_ID, "sys_device_count", count, device_name, MODEL_NAME, is_rtl=True
            )

            # B. Uptime
            uptime_s = int(time.monotonic() - start_time)
            mqtt_handler.send_sensor(
                DEVICE_ID, "sys_uptime", uptime_s, device_name, MODEL_NAME, is_rtl=True
            )
...
if __name__ == "__main__":
    ...
    threading.Thread(
        target=system_stats_loop, args=(mqtt_handler, BASE_DEVICE_ID, BASE_MODEL_NAME), daemon=True
    ).start()
