...
def test_run_script_health():
    ...
    assert os.access(script_path, os.X_OK), (
        f"CRITICAL: {script_path} is NOT executable! Run 'chmod +x {script_path}'"
    )
...
def test_dependency_files_exist():
    ...
    assert has_pyproject or has_requirements, (
        "CRITICAL: No dependency file found! (Missing pyproject.toml OR requirements.txt)"
    )
...
def test_git_security():
    ...
    assert "venv" in ignored or ".venv" in ignored, (
        "CLEANLINESS: Virtual env folders should be ignored."
    )
